...
if 'CPT' not in data.columns or 'Modifier' not in data.columns:
    ...
    with st.expander("How to use this tool"):
        st.write("""
            ...
            6. The entered data and the MDFS data will be displayed below.
        """)

    # Layout the app with input fields on the left and data on the bottom
    col1, col2 = st.columns(2)
    with col1:
        # Dynamic autocomplete for CPT code
        all_cpt_codes = data['CPT'].astype(str).unique().tolist()
        typed_cpt_code = st.text_input('Enter CPT Code')
        filtered_cpt_codes = [code for code in all_cpt_codes if typed_cpt_code in code]
        cpt_code = st.selectbox('Select CPT Code', filtered_cpt_codes) if typed_cpt_code else None
    ...
    # Display the calculated tables at the bottom
    if len(st.session_state.cpt_codes) == len(st.session_state.modifiers) == len(st.session_state.responsibilities):
        if st.session_state.cpt_codes:
            input_data = pd.DataFrame({
                'CPT Code': st.session_state.cpt_codes,
